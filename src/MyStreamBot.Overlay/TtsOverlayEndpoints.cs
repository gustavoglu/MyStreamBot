...
public static class TtsOverlayEndpoints
{
    ...
    public static void MapTtsOverlay(this WebApplication app)
    {
        app.MapPost("/api/tts", (TtsOverlayPublishRequest request) =>
        {
            ...
            if (!File.Exists(audioPath)) return Results.NotFound(new { error = "Áudio ainda não está disponível.", fileName = safeFileName });
            var id = Interlocked.Increment(ref _nextId);
            Events.Enqueue(new TtsOverlayEvent(id, request.Username.Trim(), request.AvatarUrl, request.Text.Trim(), safeFileName, DateTime.UtcNow));
            TrimEvents();
            return Results.Ok(new { id });
        });

        app.MapGet("/api/tts/current", () => Results.Ok(new { id = Volatile.Read(ref _nextId) }));
        app.MapGet("/api/tts", (long? afterId) => Results.Ok(Events.Where(x => x.Id > (afterId ?? 0)).OrderBy(x => x.Id).Take(20).ToArray()));

        app.MapPost("/api/tts/repeat-last", () =>
        {
            var item = Events.OrderByDescending(x => x.Id).FirstOrDefault();
            return item is null ? Results.NotFound(new { error = "Nenhum TTS disponível para repetir." }) : Results.Ok(EnqueueRepeat(item));
        });
        ...
        app.MapPost("/api/tts/repeat-user/{username}", (string username) =>
        {
            ...
            return item is null ? Results.NotFound(new { error = "Nenhum TTS encontrado para este viewer.", username }) : Results.Ok(EnqueueRepeat(item));
        });
    ...
    private static object EnqueueRepeat(TtsOverlayEvent original)
    {
        var id = Interlocked.Increment(ref _nextId);
