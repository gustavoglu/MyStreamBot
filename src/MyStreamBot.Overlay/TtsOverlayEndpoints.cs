...
</style></head><body><div class="wrap"><div id="card" class="card"><div class="badge">🔊 O CHAT MANDOU!</div><div class="speaker">🔈</div><div class="content"><div id="text" class="text"></div><div id="user" class="user"></div><div class="status">LENDO EM VOZ ALTA...</div></div><div class="equalizer"><i class="bar"></i><i class="bar"></i><i class="bar"></i><i class="bar"></i><i class="bar"></i><i class="bar"></i></div></div></div><audio id="audio" preload="auto"></audio><script>
let lastId=0,processing=false,queue=[],seen=new Set();const card=document.getElementById('card'),text=document.getElementById('text'),user=document.getElementById('user'),audio=document.getElementById('audio');const sleep=ms=>new Promise(r=>setTimeout(r,ms));function escapeHtml(v){return String(v??'').replace(/[&<>\"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',"'":'&#39;'}[c]))}function show(item){text.innerHTML='<span class="quote">“</span>'+escapeHtml(item.text)+'<span class="quote">”</span>';user.innerHTML='Mensagem de <strong>'+escapeHtml(item.username)+'</strong>';audio.src='/api/tts/audio/'+encodeURIComponent(item.audioFileName);card.className='card show'}async function play(item){processing=true;show(item);try{await audio.play();await new Promise(resolve=>{audio.onended=resolve;audio.onerror=resolve})}catch{await sleep(Math.max(1800,Math.min(8000,(item.text||'').length*65)))}card.className='card hide';await sleep(380);card.className='card';processing=false;processQueue()}function processQueue(){if(processing||!queue.length)return;play(queue.shift())}async function poll(){try{const response=await fetch('/api/tts?afterId='+lastId+'&ts='+Date.now(),{cache:'no-store'});if(response.ok){const items=await response.json();for(const item of items){lastId=Math.max(lastId,item.id);if(!seen.has(item.id)){seen.add(item.id);queue.push(item)}}processQueue()}}catch{}finally{setTimeout(poll,700)}}poll();
</script></body></html>
""";
