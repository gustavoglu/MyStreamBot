...
public static class TtsOverlayEndpoints
{
    ...
    public static void MapTtsOverlay(this WebApplication app)
    {
        ...
        // Retorna o maior ID atualmente existente. O overlay usa isso apenas
        // na primeira abertura para ignorar o histórico que já existia antes dele.
        // Depois disso, o próprio browser guarda o último áudio realmente ouvido.
...
</style></head><body><div class="wrap"><div id="card" class="card"><div class="badge">🔊 O CHAT MANDOU!</div><div class="speaker">🔈</div><div class="content"><div id="text" class="text"></div><div id="user" class="user"></div><div class="status">LENDO EM VOZ ALTA...</div></div><div class="equalizer"><i class="bar"></i><i class="bar"></i><i class="bar"></i><i class="bar"></i><i class="bar"></i><i class="bar"></i></div></div></div><audio id="audio" preload="auto" playsinline></audio><script>
const STORAGE_KEY='mystreambot.tts.lastPlayedId';let lastId=0,processing=false,queue=[],seen=new Set();const card=document.getElementById('card'),text=document.getElementById('text'),user=document.getElementById('user'),audio=document.getElementById('audio');const sleep=ms=>new Promise(r=>setTimeout(r,ms));function escapeHtml(v){return String(v??'').replace(/[&<>\"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',"'":'&#39;'}[c]))}function show(item){text.innerHTML='<span class="quote">“</span>'+escapeHtml(item.text)+'<span class="quote">”</span>';user.innerHTML='Mensagem de <strong>'+escapeHtml(item.username)+'</strong>';audio.pause();audio.currentTime=0;audio.src='/api/tts/audio/'+encodeURIComponent(item.audioFileName)+'?v='+encodeURIComponent(item.id);audio.load();card.className='card show'}async function play(item){processing=true;show(item);let played=false;try{audio.volume=1;await audio.play();played=await new Promise(resolve=>{const done=ok=>{audio.onended=null;audio.onerror=null;resolve(ok)};audio.onended=()=>done(true);audio.onerror=()=>done(false)})}catch(error){console.warn('[MyStreamBot TTS] Falha ao reproduzir áudio:',error);played=false}if(played){try{localStorage.setItem(STORAGE_KEY,String(item.id))}catch{}}else{console.warn('[MyStreamBot TTS] O áudio não foi marcado como ouvido. Verifique o áudio da Browser Source no OBS.');await sleep(1000)}card.className='card hide';await sleep(380);card.className='card';processing=false;if(played)processQueue();else{queue.unshift(item);setTimeout(processQueue,1500)}}function processQueue(){if(processing||!queue.length)return;play(queue.shift())}async function initialize(){try{const saved=Number.parseInt(localStorage.getItem(STORAGE_KEY)||'',10);if(Number.isFinite(saved)){lastId=Math.max(0,saved)}else{const response=await fetch('/api/tts/current?ts='+Date.now(),{cache:'no-store'});if(response.ok){const current=await response.json();lastId=Number(current.id||0);localStorage.setItem(STORAGE_KEY,String(lastId))}}}catch{lastId=0}poll()}async function poll(){try{const response=await fetch('/api/tts?afterId='+lastId+'&ts='+Date.now(),{cache:'no-store'});if(response.ok){const items=await response.json();for(const item of items){if(item.id<=lastId||seen.has(item.id))continue;seen.add(item.id);queue.push(item)}processQueue()}}catch{}finally{setTimeout(poll,700)}}initialize();
</script></body></html>
""";
