...
public static class TtsOverlayEndpoints
{
    ...
    private static readonly string ServerInstanceId = Guid.NewGuid().ToString("N");
    ...
    public static void MapTtsOverlay(this WebApplication app)
    {
        ...
        app.MapGet("/api/tts/current", async (string? client, IDbContextFactory<MyStreamBotDbContext> factory, CancellationToken ct) =>
        {
            ...
            return Results.Ok(new { client = clientName, currentId, lastEventId = state.LastEventId, serverInstanceId = ServerInstanceId });
        });
        ...
        app.MapGet("/api/tts", async (string? client, IDbContextFactory<MyStreamBotDbContext> factory, CancellationToken ct) =>
        {
            ...
            var items = await db.TtsOverlayEvents.AsNoTracking()
                .Where(x => !x.IsCanceled && x.Id > state.LastEventId)
                .OrderBy(x => x.Id)
                .Take(20)
                .ToArrayAsync(ct);
            if (items.Length > 0)
            ...
            return Results.Ok(new { serverInstanceId = ServerInstanceId, currentId, lastEventId = state.LastEventId, items });
        });
        ...
        app.MapPost("/api/tts/repeat-last", async (IDbContextFactory<MyStreamBotDbContext> factory, CancellationToken ct) =>
        {
            await EnsureSchemaAsync(factory, ct);
            await using var db = await factory.CreateDbContextAsync(ct);
            var original = await db.TtsOverlayEvents.OrderByDescending(x => x.Id).FirstOrDefaultAsync(x => !x.IsRepeat && !x.IsCanceled, ct);
        ...
        app.MapPost("/api/tts/repeat-user/{username}", async (string username, IDbContextFactory<MyStreamBotDbContext> factory, CancellationToken ct) =>
        {
            ...
            var original = await db.TtsOverlayEvents.Where(x => !x.IsRepeat && !x.IsCanceled && x.Username.ToLower() == normalized).OrderByDescending(x => x.Id).FirstOrDefaultAsync(ct);
    ...
    {
        var repeat = new TtsOverlayEvent { Username = original.Username, AvatarUrl = original.AvatarUrl, Text = original.Text, AudioFileName = original.AudioFileName, CreatedAtUtc = DateTime.UtcNow, IsRepeat = true, IsCanceled = false };
...
const explicitClient=(params.get('client')||'').trim();
const CLIENT_STORAGE_KEY='mystreambot.tts.clientId';
function getClientId(){if(explicitClient)return explicitClient.slice(0,200);try{let id=sessionStorage.getItem(CLIENT_STORAGE_KEY);if(!id){id='tts-'+crypto.randomUUID();sessionStorage.setItem(CLIENT_STORAGE_KEY,id)}return id.slice(0,200)}catch{return 'tts-'+Math.random().toString(36).slice(2)+Date.now().toString(36)}}
const CLIENT_ID=getClientId();
const SERVER_STORAGE_KEY='mystreambot.tts.serverInstanceId';
const MIN_DISPLAY_MS=5000;
let serverInstanceId=sessionStorage.getItem(SERVER_STORAGE_KEY)||'';
let lastId=0,processing=false,queue=[],seen=new Set();
const card=document.getElementById('card'),avatar=document.getElementById('avatar'),text=document.getElementById('text'),user=document.getElementById('user'),audio=document.getElementById('audio');
const sleep=ms=>new Promise(r=>setTimeout(r,ms));
function log(stage,item,extra=''){console.log(`[TTS-OVERLAY] ${stage} Client=${CLIENT_ID} EventId=${item?.id??'-'} User=${item?.username??'-'} File=${item?.audioFileName??'-'} ${extra}`)}
function escapeHtml(v){return String(v??'').replace(/[&<>\\"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\\"':'&quot;',"'":'&#39;'}[c]))}
async function ack(id,stage,error){try{await fetch('/api/tts/'+id+'/ack?client='+encodeURIComponent(CLIENT_ID),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({stage,error}),cache:'no-store'})}catch(e){console.warn(`[TTS-OVERLAY] ACK_FAILED Client=${CLIENT_ID} EventId=${id} Stage=${stage}`,e)}}
function resetPlayback(reason){audio.pause();audio.removeAttribute('src');audio.load();queue.length=0;seen.clear();processing=false;card.className='card';console.warn(`[TTS-OVERLAY] QUEUE_RESET Client=${CLIENT_ID} Reason=${reason}`)}
function show(item){text.innerHTML='<span class="quote">“</span>'+escapeHtml(item.text)+'<span class="quote">”</span>';user.innerHTML='Mensagem de <strong>'+escapeHtml(item.username)+'</strong>';avatar.src=item.avatarUrl||'';audio.pause();audio.currentTime=0;audio.src='/api/tts/audio/'+encodeURIComponent(item.audioFileName)+'?v='+encodeURIComponent(item.id);audio.load();card.className='card show';log('DISPLAYING',item)}
async function waitForAudio(){if(audio.readyState>=3)return true;return await new Promise(resolve=>{let done=false;const finish=ok=>{if(done)return;done=true;audio.removeEventListener('canplay',onReady);audio.removeEventListener('canplaythrough',onReady);audio.removeEventListener('error',onError);resolve(ok)};const onReady=()=>finish(true),onError=()=>finish(false);audio.addEventListener('canplay',onReady);audio.addEventListener('canplaythrough',onReady);audio.addEventListener('error',onError);setTimeout(()=>finish(false),8000)})}
async function play(item){processing=true;const displayStartedAt=performance.now();log('EVENT_RECEIVED',item);show(item);let played=false;try{if(!await waitForAudio()){log('AUDIO_LOAD_ERROR',item);await ack(item.id,'PLAY_ERROR','Áudio não carregou em até 8 segundos.');return}log('AUDIO_READY',item);await audio.play();played=true;log('PLAY_STARTED',item);await ack(item.id,'PLAY_STARTED');await new Promise(resolve=>{const finish=()=>{audio.removeEventListener('ended',finish);resolve()};audio.addEventListener('ended',finish);setTimeout(finish,Math.max(10000,(audio.duration||10)*1000+2000))});log('PLAY_FINISHED',item);await ack(item.id,'PLAY_FINISHED')}catch(error){console.error(`[TTS-OVERLAY] PLAY_ERROR Client=${CLIENT_ID} EventId=${item.id}`,error);await ack(item.id,'PLAY_ERROR',String(error))}finally{const remaining=Math.max(0,MIN_DISPLAY_MS-(performance.now()-displayStartedAt));if(remaining>0)await sleep(remaining);if(played)lastId=Math.max(lastId,item.id);card.className='card hide';await sleep(380);card.className='card';processing=false;processQueue()}}
function processQueue(){if(processing||!queue.length)return;play(queue.shift())}
async function initialize(){try{const response=await fetch('/api/tts/current?client='+encodeURIComponent(CLIENT_ID)+'&ts='+Date.now(),{cache:'no-store'});if(response.ok){const current=await response.json();if(serverInstanceId&&serverInstanceId!==current.serverInstanceId){resetPlayback('server-restart');}serverInstanceId=current.serverInstanceId||'';if(serverInstanceId)sessionStorage.setItem(SERVER_STORAGE_KEY,serverInstanceId);lastId=Number(current.lastEventId||0);console.log(`[TTS-OVERLAY] INITIALIZED Client=${CLIENT_ID} LastId=${lastId} CurrentId=${current.currentId} Server=${serverInstanceId}`)}}catch(error){console.warn('[TTS-OVERLAY] INITIALIZE_ERROR',error)}poll()}
async function poll(){try{const response=await fetch('/api/tts?client='+encodeURIComponent(CLIENT_ID)+'&ts='+Date.now(),{cache:'no-store'});if(response.ok){const data=await response.json();if(serverInstanceId&&data.serverInstanceId&&data.serverInstanceId!==serverInstanceId){resetPlayback('server-restart');serverInstanceId=data.serverInstanceId;sessionStorage.setItem(SERVER_STORAGE_KEY,serverInstanceId);lastId=Number(data.lastEventId||data.currentId||0);return}if(!serverInstanceId&&data.serverInstanceId){serverInstanceId=data.serverInstanceId;sessionStorage.setItem(SERVER_STORAGE_KEY,serverInstanceId)}for(const item of (data.items||[])){if(item.id<=lastId||seen.has(item.id))continue;seen.add(item.id);queue.push(item);log('QUEUED',item)}processQueue()}}catch(error){console.warn('[TTS-OVERLAY] POLL_ERROR',error)}finally{setTimeout(poll,1000)}}
initialize();
</script></body></html>
""";
