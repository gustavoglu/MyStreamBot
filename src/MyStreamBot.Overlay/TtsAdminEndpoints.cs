...
public static class TtsAdminEndpoints
{
    public static void MapTtsAdmin(this WebApplication app)
    {
        ...
        {
            await using var db = await factory.CreateDbContextAsync(ct);
            var settings = await GetSettingsAsync(db, ct); settings.AcceptCommands = request.AcceptCommands; settings.PublishEnabled = request.PublishEnabled; settings.UpdatedAtUtc = DateTime.UtcNow; await db.SaveChangesAsync(ct); return Results.Ok(new { settings.AcceptCommands, settings.PublishEnabled });
        });
        ...
        app.MapGet("/api/admin/users", async (string? q, IDbContextFactory<MyStreamBotDbContext> factory, CancellationToken ct) =>
        {
            ...
            return Results.Ok(await query.OrderByDescending(x => x.Points).Take(100).Select(x => new { x.Id, x.Username, x.AvatarUrl, Points = x.Points, Platform = x.Platform.ToString(), x.PlatformUserId }).ToListAsync(ct));
        });
