...
function render(){updates.innerHTML='';if(!latest.length){updates.innerHTML='<div class="empty">Aguardando atualizações...</div>';return}for(const item of latest){if(!item.user)continue;const row=document.createElement('div');row.className='update';row.innerHTML=`<img class="avatar" src="${item.user.avatarUrl||fallbackAvatar}" onerror="this.src='${fallbackAvatar}'"><div class="info"><div class="name">${escapeHtml(item.user.username)}</div><div class="type">${typeLabel(item.type)}</div></div><div class="reward"><div class="total-now">(${formatPoints(item.user.points)})</div><div class="gain">+${formatPoints(item.amount)}</div></div>`;updates.appendChild(row)}}
async function cycle(){while(true){render();panel.classList.remove('fade-out');title.className='title';subtitle.className='subtitle';[...updates.children].forEach(x=>x.className='update');await sleep(120);title.classList.add('show-title');await sleep(180);subtitle.classList.add('show-title');await sleep(180);for(const row of [...updates.children]){row.classList.add('show-line');await sleep(150)}await sleep(5000);panel.classList.add('fade-out');await sleep(700);panel.classList.remove('fade-out');title.className='title';subtitle.className='subtitle';[...updates.children].forEach(x=>x.className='update');await sleep(10000)}}
async function refresh(){if(refreshing)return;refreshing=true;try{const response=await fetch('/api/recent?ts='+Date.now(),{cache:'no-store'});if(response.ok)latest=await response.json()}catch{}finally{refreshing=false}}refresh();setInterval(refresh,2000);cycle();
</script></body></html>
""";
...
function render(){top.innerHTML='';if(!latest.length){top.innerHTML='<div class="empty">Aguardando pontuação...</div>';return}latest.forEach((u,i)=>{const row=document.createElement('div');row.className='item';row.innerHTML=`<div class="rank">${medals[i]||i+1}</div><img class="avatar" src="${escapeHtml(u.avatarUrl||fallbackAvatar)}" onerror="this.src='${fallbackAvatar}'"><div class="info"><div class="name">${escapeHtml(u.username)}</div><div class="label">${i+1}º lugar</div></div><div class="points">${formatPoints(u.points)}</div>`;top.appendChild(row)})}
async function cycle(){while(true){render();panel.classList.remove('fade-out');title.className='title';[...top.children].forEach(x=>x.className='item');await sleep(120);title.classList.add('show-title');await sleep(220);for(const row of [...top.children]){row.classList.add('show-line');await sleep(180)}await sleep(5000);panel.classList.add('fade-out');await sleep(700);panel.classList.remove('fade-out');title.className='title';[...top.children].forEach(x=>x.className='item');await sleep(10000)}}
async function refresh(){if(refreshing)return;refreshing=true;try{const response=await fetch('/api/top?ts='+Date.now(),{cache:'no-store'});if(response.ok)latest=await response.json()}catch{}finally{refreshing=false}}refresh();setInterval(refresh,2000);cycle();
</script></body></html>
""";
