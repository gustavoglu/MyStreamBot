...
builder.Services.AddHostedService<TtsStartupReconciler>();
...
app.MapGet("/recents", () => Results.Content(RecentsCarouselHtml(), "text/html; charset=utf-8"));
...
<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>MyStreamBot - Últimas movimentações</title><style>:root{color-scheme:dark}*{box-sizing:border-box}html,body{margin:0;padding:0;background:transparent;font-family:Arial,Helvetica,sans-serif;color:#fff}body{width:460px;overflow:hidden}.panel{padding:14px;background:transparent}.title,.subtitle,.update{opacity:0;transform:translateX(-90px)}.title{font-size:19px;font-weight:800;margin:0 0 10px}.subtitle{color:#aeb5c2;font-size:12px;margin:-6px 0 12px}.update{display:flex;align-items:center;gap:11px;min-height:66px;padding:9px 10px;margin:7px 0;border-radius:13px;background:transparent;overflow:hidden}.avatar{width:44px;height:44px;border-radius:50%;object-fit:cover;background:transparent;flex:0 0 44px}.info{min-width:0;flex:1}.name{font-size:14px;font-weight:750;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.type{color:#8993a3;font-size:10px;text-transform:uppercase;letter-spacing:.7px;margin-top:4px}.reward{display:flex;align-items:baseline;justify-content:flex-end;gap:8px;flex:0 0 auto;white-space:nowrap}.total-now{color:#d5dbe5;font-size:19px;font-weight:900}.gain{font-size:19px;font-weight:900;color:#6dff9b}.gain.spend{color:#ff7272}.empty{color:#8f98a7;font-size:13px;padding:16px 4px}.show-title,.show-line{animation:slideIn .55s cubic-bezier(.22,.8,.24,1) forwards}.fade-out{animation:fadeOut .7s ease forwards!important}@keyframes slideIn{from{opacity:0;transform:translateX(-90px)}to{opacity:1;transform:translateX(0)}}@keyframes fadeOut{from{opacity:1}to{opacity:0}}</style></head><body><div class="panel" id="panel"><div class="title" id="title">⚡ Últimas movimentações</div><div class="subtitle" id="subtitle">Pontos recebidos e gastos pelos viewers</div><div id="updates"></div></div><script>
const fallbackAvatar='data:image/svg+xml;charset=UTF-8,'+encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="80" height="80"><rect width="100%" height="100%" fill="transparent"/><circle cx="40" cy="31" r="14" fill="#9aa3b2"/><path d="M15 72c3-17 47-17 50 0" fill="#9aa3b2"/></svg>');let latest=[];let refreshing=false;const panel=document.getElementById('panel'),title=document.getElementById('title'),subtitle=document.getElementById('subtitle'),updates=document.getElementById('updates');function sleep(ms){return new Promise(r=>setTimeout(r,ms))}function formatPoints(v){return Number(Math.abs(v||0)).toLocaleString('pt-BR')}function typeLabel(t){const l={MessageReward:'MENSAGEM',Follow:'FOLLOW',Subscription:'INSCRIÇÃO',Gift:'GIFT',Donation:'DOAÇÃO',Bonus:'BÔNUS',Admin:'ADMIN',Penalty:'PENALIDADE',BetWin:'APOSTA',Tts:'VOZ'};return l[t]||String(t||'PONTOS').toUpperCase()}function escapeHtml(v){return String(v??'').replace(/[&<>\"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',"'":'&#39;'}[c]))}function render(){updates.innerHTML='';if(!latest.length){updates.innerHTML='<div class="empty">Nenhuma movimentação ainda.</div>';return}for(const item of latest){if(!item.user)continue;const spent=Number(item.amount)<0;const row=document.createElement('div');row.className='update';row.innerHTML=`<img class="avatar" src="${item.user.avatarUrl||fallbackAvatar}" onerror="this.src='${fallbackAvatar}'"><div class="info"><div class="name">${escapeHtml(item.user.username)}</div><div class="type">${typeLabel(item.type)}${spent?' • GASTO':''}</div></div><div class="reward"><div class="total-now">(${Number(item.user.points||0).toLocaleString('pt-BR')})</div><div class="gain ${spent?'spend':''}">${spent?'-':'+'}${formatPoints(item.amount)}</div></div>`;updates.appendChild(row)}}async function refresh(){if(refreshing)return false;refreshing=true;try{const response=await fetch('/api/recent?ts='+Date.now(),{cache:'no-store'});if(!response.ok)return false;latest=await response.json();return true}catch{return false}finally{refreshing=false}}async function poll(){while(true){await refresh();await sleep(2000)}}async function cycle(){await refresh();while(true){render();panel.classList.remove('fade-out');title.className='title';subtitle.className='subtitle';[...updates.children].forEach(x=>x.className='update');await sleep(80);title.classList.add('show-title');await sleep(160);subtitle.classList.add('show-title');await sleep(160);for(const row of [...updates.children]){row.classList.add('show-line');await sleep(150)}await sleep(10000);panel.classList.add('fade-out');await sleep(700);panel.classList.remove('fade-out');title.className='title';subtitle.className='subtitle';[...updates.children].forEach(x=>x.className='update');await sleep(10000)}}poll();cycle();
</script></body></html>
""";

static string RecentsCarouselHtml() => """
<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>MyStreamBot - Pontos Recentes</title><style>:root{color-scheme:dark}*{box-sizing:border-box}html,body{margin:0;padding:0;background:transparent;font-family:Arial,Helvetica,sans-serif;color:#fff}body{width:470px;height:190px;overflow:hidden}.wrap{width:100%;height:100%;display:flex;align-items:center;justify-content:center}.card{width:430px;min-height:135px;padding:18px 22px;border-radius:20px;background:transparent;display:flex;align-items:center;gap:15px;opacity:0;transform:translateX(-70px)}.card.show{animation:in .5s cubic-bezier(.2,.8,.25,1) forwards}.card.hide{animation:out .4s ease forwards}.title{text-align:center;font-size:32px;line-height:.98;font-weight:950;letter-spacing:1px;color:#ffc12f;text-shadow:0 0 22px rgba(255,193,47,.28)}.avatar{width:68px;height:68px;border-radius:50%;object-fit:cover;background:transparent;border:2px solid rgba(255,193,47,.75);flex:0 0 68px}.info{min-width:0;flex:1}.name{font-size:23px;font-weight:900;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.type{font-size:11px;color:#9ea7b6;margin-top:5px;text-transform:uppercase;letter-spacing:1px}.points{font-size:27px;font-weight:950;white-space:nowrap;color:#6dff9b}.points.spend{color:#ff7272}@keyframes in{from{opacity:0;transform:translateX(-70px)}to{opacity:1;transform:translateX(0)}}@keyframes out{from{opacity:1;transform:translateX(0)}to{opacity:0;transform:translateX(55px)}}</style></head><body><div class="wrap"><div id="root"></div></div><script>
const SHOW_MS=4000,TITLE_MS=1800,OUT_MS=400;const fallbackAvatar='data:image/svg+xml;charset=UTF-8,'+encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="80" height="80"><rect width="100%" height="100%" fill="transparent"/><circle cx="40" cy="31" r="14" fill="#9aa3b2"/><path d="M15 72c3-17 47-17 50 0" fill="#9aa3b2"/></svg>');const root=document.getElementById('root');const sleep=ms=>new Promise(r=>setTimeout(r,ms));let latest=[];let refreshing=false;function esc(v){return String(v??'').replace(/[&<>\"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',"'":'&#39;'}[c]))}function typeLabel(t){const l={MessageReward:'MENSAGEM',Follow:'FOLLOW',Subscription:'INSCRIÇÃO',Gift:'GIFT',Donation:'DOAÇÃO',Bonus:'BÔNUS',Admin:'ADMIN',Penalty:'PENALIDADE',BetWin:'APOSTA',Tts:'VOZ'};return l[t]||String(t||'PONTOS').toUpperCase()}async function refresh(){if(refreshing)return;refreshing=true;try{const r=await fetch('/api/recent?ts='+Date.now(),{cache:'no-store'});if(r.ok)latest=await r.json()}catch(e){console.warn('[RECENTS-CAROUSEL] POLL_ERROR',e)}finally{refreshing=false}}function titleCard(){root.innerHTML='<div class="card" id="card"><div class="title">Pontos<br>Recentes</div></div>';const c=document.getElementById('card');requestAnimationFrame(()=>c.className='card show');return c}function itemCard(item){const u=item.user||{};const spent=Number(item.amount)<0;root.innerHTML=`<div class="card" id="card"><img class="avatar" src="${esc(u.avatarUrl||fallbackAvatar)}" onerror="this.src='${fallbackAvatar}'"><div class="info"><div class="name">${esc(u.username||'Viewer')}</div><div class="type">${typeLabel(item.type)}${spent?' • GASTO':''}</div></div><div class="points ${spent?'spend':''}">${spent?'-':'+'}${Math.abs(Number(item.amount||0)).toLocaleString('pt-BR')}</div></div>`;const c=document.getElementById('card');requestAnimationFrame(()=>c.className='card show');return c}async function showCard(card,ms){await sleep(ms);card.className='card hide';await sleep(OUT_MS);root.innerHTML=''}async function cycle(){while(true){await refresh();const valid=latest.filter(x=>x.user);if(!valid.length){await sleep(2000);continue}const title=titleCard();await showCard(title,TITLE_MS);for(const item of valid.slice(0,5)){const card=itemCard(item);await showCard(card,SHOW_MS)}await sleep(1000)}}refresh();cycle();setInterval(refresh,2000);
</script></body></html>
""";
