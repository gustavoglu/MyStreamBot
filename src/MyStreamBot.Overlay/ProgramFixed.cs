...
<body>
<div class="panel" id="panel"><div class="title" id="title">⚡ Últimas atualizações</div><div class="subtitle" id="subtitle">Pontos recebidos pelos viewers</div><div id="updates"></div></div>
<script>
...
let latest=[];let refreshing=false;
const panel=document.getElementById('panel'),title=document.getElementById('title'),subtitle=document.getElementById('subtitle'),updates=document.getElementById('updates');
...
<!doctype html>
<html lang="pt-BR">
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>MyStreamBot - Top 5</title>
...
const fallbackAvatar='data:image/svg+xml;charset=UTF-8,'+encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="80" height="80"><rect width="100%" height="100%" fill="#303540"/><circle cx="40" cy="31" r="14" fill="#9aa3b2"/><path d="M15 72c3-17 47-17 50 0" fill="#9aa3b2"/></svg>');const queue=[],queuedIds=new Set(),seenIds=new Set();let initialized=false,processing=false,refreshing=false;
function formatPoints(v){return Number(v||0).toLocaleString('pt-BR')}function setPopup(item){const u=item.user;if(!u)return;const avatar=document.getElementById('avatar');avatar.src=u.avatarUrl||fallbackAvatar;avatar.onerror=()=>avatar.src=fallbackAvatar;document.getElementById('name').textContent=u.username||'Viewer';document.getElementById('message').textContent=`Total: ${formatPoints(u.points)} pontos`;document.getElementById('gain').textContent=`+${formatPoints(item.amount)}`;const popup=document.getElementById('popup');popup.classList.remove('show');void popup.offsetWidth;popup.classList.add('show')}
function sleep(ms){return new Promise(resolve=>setTimeout(resolve,ms))}async function processQueue(){if(processing)return;processing=true;while(queue.length){const item=queue.shift();queuedIds.delete(String(item.id));setPopup(item);await sleep(2000);document.getElementById('popup').classList.remove('show');await sleep(220)}processing=false}
async function refresh(){if(refreshing)return;refreshing=true;try{const response=await fetch('/api/recent?ts='+Date.now(),{cache:'no-store'});if(!response.ok)return;const items=await response.json();if(!items.length)return;if(!initialized){for(const item of items)seenIds.add(String(item.id));initialized=true;return}const fresh=items.filter(x=>!seenIds.has(String(x.id))).reverse();for(const item of fresh){const id=String(item.id);seenIds.add(id);if(!queuedIds.has(id)){queue.push(item);queuedIds.add(id)}}while(seenIds.size>100)seenIds.delete(seenIds.values().next().value);processQueue()}catch{}finally{refreshing=false}}
async function poll(){while(true){await refresh();await sleep(2000)}}poll();
</script></body></html>
""";